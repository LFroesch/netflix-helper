...
const SearchPage = () => {
    ...
    const performSearch = async (tab, term) => {
        try {
            const res = await axios.get(`/api/v1/search/${tab}/${term}`);
    ...
    return <div className='bg-black min-h-screen text-white'>
			...
			<div className='container mx-auto px-4 py-8'>
				...
				<div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
					{results.map((result) => {
						if (!result.poster_path && !result.profile_path) return null; // Skip if no image
						return (
